...
export default function AutoRechargeMappingEditPage() {
  const router = useRouter()
  const params = useParams()
  const mappingUid = params.uid as string
  ...
  useEffect(() => {
    const fetchMapping = async () => {
      setFetching(true)
      try {
        // First, get all mappings to find the one with this UID
        const data = await apiFetch(`${baseUrl}/api/auto-recharge/admin/mappings/`)
        const allMappings = data.results || []
        const mapping = allMappings.find((m: any) => m.uid === mappingUid)
        
        if (mapping) {
          setNetwork(mapping.network?.uid || "")
          setAggregator(mapping.aggregator?.uid || "")
          setIsActive(mapping.is_active || false)
          setPriority(mapping.priority?.toString() || "0")
          setFixedFee(mapping.fixed_fee || "0.00")
          setPercentageFee(mapping.percentage_fee || "0.00")
          setMinAmount(mapping.min_amount || "100.00")
        ...
        toast({
          title: t("autoRecharge.mapping.notFound"),
          description: t("autoRecharge.mapping.notFound"),
          variant: "destructive",
        })
      }
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiFetch(`${baseUrl}/api/auto-recharge/admin/mappings/${mappingUid}/update/`, {
        method: "PATCH",
      ...
      setError(errorMessage)
      toast({
        title: t("autoRecharge.mapping.failedToUpdate"),
        description: errorMessage,
        variant: "destructive",
      })
    } finally {
  ...
  if (fetching) {
    return (
      <div className="flex items-center justify-center h-96">
        <span className="text-lg font-semibold">{t("autoRecharge.mapping.loading")}</span>
      </div>
    )
  }
  ...
  if (loading && !error) {
    return (
      <div className="flex items-center justify-center h-96">
        <span className="text-lg font-semibold">{t("autoRecharge.mapping.updating")}</span>
      </div>
    )
  }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("autoRecharge.mapping.edit")}</CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-4">
            <Button type="submit" disabled={loading}>
              {loading ? t("autoRecharge.mapping.updating") : t("autoRecharge.mapping.updateMapping")}
            </Button>
            <Button
              type="button"
              variant="outline"
